...
WORKSPACE_NAMES = {"Kunal Operations", "Kunal Admin"}
...
class TestWorkspaceNavigation(FrappeTestCase):
	def test_workspace_fixture_is_registered(self):
		...
		self.assertEqual(
			...
			[["name", "in", ["Kunal Operations", "Kunal Admin"]]],
		)
	...
	def test_workspace_fixture_contains_expected_records_and_metadata(self):
		...
		self.assert_workspace_metadata(workspaces["Kunal Operations"], {"Owner", "Admin", "Branch Manager", "Branch Employee"})
		self.assert_workspace_metadata(workspaces["Kunal Admin"], {"Owner", "Admin"})

	def test_workspace_shortcuts_match_approved_scope(self):
		workspaces = self._workspace_fixtures()

		self.assert_workspace_shortcuts(workspaces["Kunal Operations"], OPERATION_SHORTCUTS)
		self.assert_workspace_shortcuts(workspaces["Kunal Admin"], ADMIN_SHORTCUTS)
	...
	def test_admin_workspace_targets_have_read_only_owner_admin_permissions(self):
		...
		for doctype in ADMIN_NAVIGATION_DOCTYPES:
			for role in ("Owner", "Admin"):
				permission = custom_docperms.get((doctype, role))
	...
	def test_boot_workspace_filter_does_not_touch_system_manager_or_unrelated_users(self):
		all_workspaces = ["Kunal Operations", "Kunal Admin", "Users", "Website", "Tools"]
	...
	def test_sidebar_api_filter_uses_same_kunal_role_rules(self):
		...
		self.assert_workspace_page_filter(["Owner"], all_workspaces, ["Kunal Operations", "Kunal Admin"])
		self.assert_workspace_page_filter(["Admin"], all_workspaces, ["Kunal Operations", "Kunal Admin"])
		self.assert_workspace_page_filter(["Branch Manager"], all_workspaces, ["Kunal Operations"])
		self.assert_workspace_page_filter(["Branch Employee"], all_workspaces, ["Kunal Operations"])
	...
	def assert_workspace_metadata(self, workspace, expected_roles):
		...
		self.assertEqual({row["role"] for row in workspace["roles"]}, expected_roles)
	...
	def _custom_docperm_fixtures(self):
		...
		with fixture_path.open() as fixture_file:
			return {
				(permission["parent"], permission["role"]): permission
	...
	def assert_workspace_filter(self, roles, expected_workspaces):
		bootinfo = frappe._dict(
			allowed_workspaces=[
				...
				for name in ("Kunal Operations", "Kunal Admin", "Users", "Website", "Tools")
			]
		)
